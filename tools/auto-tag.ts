...
if (!getApps().length) {
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    ...
    throw new Error(
      "GOOGLE_APPLICATION_CREDENTIALS or FIREBASE_SERVICE_ACCOUNT is required",
    );
  }
}
...
function uniqLower(a: string[]) {
  return Array.from(
    new Set(a.map((s) => s.trim().toLowerCase()).filter(Boolean)),
  );
}
...
function deriveTags(storagePath?: string, fileUrl?: string, title?: string) {
  ...
  for (const r of PATH_RULES)
    if (r.test(target)) r.tags.forEach((t) => hits.add(t));
...
async function main() {
  ...
  console.log(
    `done. docs=${snap.size}, changed=${changed}, skipped=${skipped}, dry=${DRY_RUN}`,
  );
}
